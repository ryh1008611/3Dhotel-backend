...
class BaseController extends Controller {
  ...
  async index() {
    ...
    const result = await service[this.app.config.version].admin[this.modleName][this.serviceName].findList(query);
  ...
  async show() {
    ...
    const result = await service[this.app.config.version].admin[this.modleName][this.serviceName].findByPk(id);
    ctx.returnBody(result, 100010);
  }
  ...
  async create() {
    ...
    const result = await service[this.app.config.version].admin[this.modleName][this.serviceName].create(query);
    if (result) {
  ...
  async update() {
    ...
    const result = await service[this.app.config.version].admin[this.modleName][this.serviceName].update(query, {
      id,
    });
    if (result) {
  ...
  async destroy() {
    ...
    const result = await service[this.app.config.version].admin[this.modleName][this.serviceName].destroy(ids);
